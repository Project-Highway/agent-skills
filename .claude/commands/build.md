...
Invoke the highway-skills:incremental-implementation skill alongside highway-skills:test-driven-development.
...
3. **Plan if needed.** If there is no `tasks/plan.md`, invoke highway-skills:planning-and-task-breakdown to generate one.
...
6. **Stop and ask the user** (do not push through) when:
   - a test can't be made to pass or the build breaks without an obvious fix → follow highway-skills:debugging-and-error-recovery
   - the spec is ambiguous, or a task needs a decision the spec doesn't cover
   - a task is high-risk or irreversible — auth/permission changes, destructive data migrations, payments, deletions, deploys, anything touching secrets, **or anything you can't undo with `git revert`** → follow highway-skills:doubt-driven-development and get explicit sign-off before continuing
...
If any step fails, follow the highway-skills:debugging-and-error-recovery skill.
